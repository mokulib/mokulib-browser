...
const endTimeInput = ref<string>(DateTime.now().toISO().slice(0, 16));
const statusInput = ref<'RETURNED' | 'LOST' | 'DAMAGED'>('RETURNED');
...
onMounted(() => {
  usePopupStore().registerInitHook('returnBook', ({ clone }) => {
    ...
    // 刷新弹窗输入
    endTimeInput.value = DateTime.now().toISO().slice(0, 16);
...
<template>
  <Popup popup-key="returnBook" title="归还馆藏" confirm-text="确认归还" :confirm-handler="() => api.post<BookCopyAdmin>(`/api/borrow-records/${borrowRecordId}/return`, { status: statusInput, end_time: endTimeInput + ':00' })">
      ...
      <div class="space-y-1.5">
        ...
        <div class="flex flex-col gap-2">
          <label class="flex items-center gap-2 text-sm text-(--foreground)">
            <input v-model="statusInput" value="RETURNED" class="size-4 border-(--border) accent-(--primary)" type="radio" checked name="return-status-102">
            正常归还
          </label>
          <label class="flex items-center gap-2 text-sm text-(--foreground)">
            <input v-model="statusInput" value="LOST" class="size-4 border-(--border) accent-(--primary)" type="radio" name="return-status-102">
            丢失
          </label>
          <label class="flex items-center gap-2 text-sm text-(--foreground)">
            <input v-model="statusInput" value="DAMAGED" class="size-4 border-(--border) accent-(--primary)" type="radio" name="return-status-102">
            损坏
          </label>
        </div>
      </div>
