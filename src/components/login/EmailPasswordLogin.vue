  ...
  <form class="flex flex-col gap-5" @submit.prevent="handleLogin">
    ...
    <div class="flex flex-col gap-2">
      <div class="flex items-center justify-between">
        ...
        <button type="button" class="text-xs text-(--muted-foreground) transition-colors hover:text-(--primary)" @click="showPassword = !showPassword">{{ showPassword ? '隐藏' : '显示' }}</button>
      </div>
      <input id="password" v-model="loginForm.password" data-slot="input" placeholder="输入密码" pattern="^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*_:;,.?])[a-zA-Z\d!@#$%^&*_:;,.?]{8,20}$" maxlength="20" title="密码需 8-20 位，包含大小写字母、数字和特殊字符" autocomplete="current-password" required class="h-8 w-full min-w-0 rounded-lg border border-(--input) bg-transparent px-2.5 py-1 text-base transition-colors outline-none file:inline-flex file:h-6 file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-(--foreground) placeholder:text-(--muted-foreground) focus-visible:border-(--ring) focus-visible:ring-3 focus-visible:ring-(--ring)/50 disabled:pointer-events-none disabled:cursor-not-allowed disabled:bg-(--input)/50 disabled:opacity-50 aria-invalid:border-(--destructive) aria-invalid:ring-3 aria-invalid:ring-(--destructive)/20 md:text-sm dark:bg-(--input)/30 dark:disabled:bg-(--input)/80 dark:aria-invalid:border-(--destructive)/50 dark:aria-invalid:ring-(--destructive)/40" :type="showPassword ? 'text' : 'password'">
    </div>
    <div class="flex flex-col gap-2">
    ...
    <label class="flex items-start gap-2 text-sm text-(--muted-foreground)">
      ...
      <span>我已阅读并同意<RouterLink :to="{ name: 'terms' }" class="text-(--primary) transition-colors hover:opacity-80">《墨库用户协议》</RouterLink>、<RouterLink :to="{ name: 'privacy' }" class="text-(--primary) transition-colors hover:opacity-80">《隐私政策》</RouterLink>，并授权墨库使用该墨库账号信息（如昵称、头像、个性签名）进行统一管理</span>
    </label>
  </form>
</template>
