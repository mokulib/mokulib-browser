    ...
    <div class="border-b border-(--primary) text-sm"><!-- 全宽下边框 -->
      <div class="mx-auto max-w-6xl w-full flex items-center px-4 md:px-8"><!-- 限制宽度 -->
        ...
        <div class="flex items-center px-4 gap-4">
          <div @mouseenter="setActive('hot')" :data-is-active="isHotActive" class="mt-1 px-4 pt-1 pb-2 rounded-t data-[is-active=true]:text-(--primary) data-[is-active=true]:bg-(--primary)/10 transition-all cursor-pointer">热门好书</div>
          <div @mouseenter="setActive('new')" :data-is-active="isNewActive" class="mt-1 px-4 pt-1 pb-2 rounded-t data-[is-active=true]:text-(--primary) data-[is-active=true]:bg-(--primary)/10 transition-all cursor-pointer">新书上架</div>
        </div>
      </div>
    </div>
      ...
      <div class="flex-1 flex">
        ...
        <section class="flex-1 flex flex-col">
          ...
          <div v-if="activeCategory && activeCategory.books.total > 0" class="flex items-center pl-4 py-2 gap-4">
            <button @click="() => goToPage(1, activeCategory?.sortMode === 'PUBLISH_DATE_FROM_NEW_TO_OLD' ? 'PUBLISH_DATE_FROM_OLD_TO_NEW' : 'PUBLISH_DATE_FROM_NEW_TO_OLD')" class="flex items-center gap-2 cursor-pointer" :class="{ 'text-(--primary)': activeCategory?.sortMode === 'PUBLISH_DATE_FROM_NEW_TO_OLD' || activeCategory?.sortMode === 'PUBLISH_DATE_FROM_OLD_TO_NEW' }">
            ...
            <button @click="() => goToPage(1, activeCategory?.sortMode === 'PRICE_FROM_LOW_TO_HIGH' ? 'PRICE_FROM_HIGH_TO_LOW' : 'PRICE_FROM_LOW_TO_HIGH')" class="flex items-center gap-2 cursor-pointer" :class="{ 'text-(--primary)': activeCategory?.sortMode === 'PRICE_FROM_HIGH_TO_LOW' || activeCategory?.sortMode === 'PRICE_FROM_LOW_TO_HIGH' }">
