...
const isMyBorrowRecord = (borrowRecord: BorrowRecord | null) => {
  return borrowRecord?.user_id === authStore.id;
}
      ...
      <div class="flex flex-col gap-8 md:flex-row">
        ...
        <div class="mx-auto w-full max-w-64 shrink-0 md:mx-0">
          <div class="relative aspect-3/4 overflow-hidden rounded-lg shadow-md ring-1 ring-(--border)">
            ...
            <div class="absolute right-2 bottom-2 flex flex-col gap-2">
              <button v-if="authStore.isAdmin" type="button" @click="popupStore.open('uploadBookCover', { id }, uploadBookCoverCallback)" class="
        ...
        <div class="min-w-0 flex-1">
          <span class="inline-flex items-center gap-1 rounded-full bg-(--accent) px-2.5 py-0.5 text-xs font-medium text-(--accent-foreground)">
            {{ category?.name }}
            <button v-if="authStore.isAdmin" type="button" @click="popupStore.open('editCategory', { book, category }, editCategoryCallback)" aria-label="编辑分类" class="ml-0.5 text-(--accent-foreground)/70 hover:text-(--accent-foreground)">
              <Pencil class="size-3"/>
            </button>
          </span>
          ...
          <div class="mt-5 flex flex-wrap items-center gap-2">
            ...
            <template v-for="tag in tags" :key="tag.id">
              <span class="inline-flex items-center gap-1 rounded-full bg-(--secondary) px-2.5 py-0.5 text-xs text-(--secondary-foreground)">
                {{ tag.name }}
                <button v-if="authStore.isAdmin" type="button" @click="deleteTagHandler(tag.id)" class="text-(--secondary-foreground)/60 hover:text-(--destructive)">
                  <X class="size-3"/>
                </button>
              </span>
            </template>
            <!-- 添加标签 -->
            <button v-if="authStore.isAdmin" type="button" @click="popupStore.open('addTag', { id, tags }, addTagCallback)" class="inline-flex items-center gap-1 rounded-full border border-dashed border-(--border) px-2.5 py-0.5 text-xs text-(--muted-foreground) transition-colors hover:border-(--primary) hover:text-(--primary)">
              <Plus class="size-3"/>
              添加标签
            </button>
          </div>
          ...
          <div class="flex items-baseline gap-4">
            ...
            <div class="flex items-center">
              ...
              <div class="hidden md:flex md:items-center">
                ...
                <span v-if="authStore.isAdmin && withdrawnCount" class="text-sm text-(--muted-foreground) whitespace-pre-wrap"> · 已下架 {{ withdrawnCount }} 本</span>
              </div>
            </div>
          </div>
          <button v-if="authStore.isAdmin" type="button" @click="popupStore.open('addBookCopy', { id }, addBookCopyCallback)" class="group/button inline-flex items-center justify-center border bg-clip-padding font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-(--ring) focus-visible:ring-3 focus-visible:ring-(--ring)/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-(--destructive) aria-invalid:ring-3 aria-invalid:ring-(--destructive)/20 dark:aria-invalid:border-(--destructive)/50 dark:aria-invalid:ring-(--destructive)/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 border-(--border) bg-(--muted)/30 hover:bg-(--muted) hover:text-(--foreground) aria-expanded:bg-(--muted) aria-expanded:text-(--foreground) dark:border-(--input) dark:bg-(--input)/30 dark:hover:bg-(--input)/50 h-7 rounded-[min(var(--radius-md),12px)] px-2.5 text-[0.8rem] in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 [&_svg:not([class*='size-'])]:size-3.5 shrink-0 gap-1.5">
            <Plus class="size-3"/>
            添加馆藏
          </button>
        </div>
        <!-- 游客 -->
        <div v-if="!authStore.isAuthed" class="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-(--border) py-14 text-center">
          ...
          <template v-for="bookCopy in bookCopies" :key="bookCopy.id">
            <div class="flex flex-col rounded-xl border p-4 transition-colors border-(--border)" :class="{
              ...
              <div class="mt-4 flex flex-wrap gap-2">
                ...
                <!-- 已借出状态按钮 - 归还 -->
                <button v-if="authStore.isAdmin && bookCopy.role === 'ADMIN' && bookCopy.status === 'UNAVAILABLE'" type="button" @click="popupStore.open('returnBook', { id: bookCopy.current_borrow_record.id }, returnBookCallback)" tabindex="0" data-slot="button" class="group/button inline-flex shrink-0 items-center justify-center border bg-clip-padding font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-(--ring) focus-visible:ring-3 focus-visible:ring-(--ring)/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-(--destructive) aria-invalid:ring-3 aria-invalid:ring-(--destructive)/20 dark:aria-invalid:border-(--destructive)/50 dark:aria-invalid:ring-(--destructive)/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 border-(--border) bg-(--background) hover:bg-(--muted) hover:text-(--foreground) aria-expanded:bg-(--muted) aria-expanded:text-(--foreground) dark:border-(--input) dark:bg-(--input)/30 dark:hover:bg-(--input)/50 h-7 rounded-[min(var(--radius-md),12px)] px-2.5 text-[0.8rem] in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-1.5 has-data-[icon=inline-start]:pl-1.5 [&_svg:not([class*='size-'])]:size-3.5 gap-1.5">
